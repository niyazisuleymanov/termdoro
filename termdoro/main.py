...
def parse_timestr(timestr):
    ...
    if timedelta_secs:
        ...
        # When I do "termdoro 10" (the two cases above), I want a
        # countdown for the next 10 seconds. Okay. But when I do
        # "termdoro 23:52", I want a countdown that ends at that exact
        ...
        while seconds_left > 0 or blink or text:
            ...
            if sleep_target > now and seconds_left > 0:
                ...
                if input_action == INPUT_EXIT:  # no elif here! input_action may have changed
                    return int(ceil((target - datetime.now()).total_seconds()))
...
@click.option("--stopwatch", default=False, is_flag=True,
              help="Use stopwatch mode")
@click.option("-t", "--text",
...
def main(**kwargs):
    ...
    if kwargs['config']:
        cli = Numbers('Sessions: ')
        sessions = cli.launch()
        cli = Numbers('Work time (in mins): ')
        work_time = cli.launch()
        cli = Numbers('Break time (in mins): ')
        break_time = cli.launch()
        cli = Numbers('Long break time (in mins): ')
        long_break = cli.launch()
        cli = Numbers('Long break timing: ')
        ...
        try:
            db.addTimeElapsed(
                seconds=float(kwargs['timespec']) - time_left, 
                start_time=start_time
            )
        except: 
            db.addTimeElapsed(
                seconds=float(kwargs['timespec']), 
                start_time=start_time
            )
    elif kwargs['stopwatch']:
        seconds_elapsed, laps = curses.wrapper(stopwatch, **kwargs)
        db.addTimeElapsed(seconds_elapsed, start_time)
        ...
        LONG_BREAK_TIMING = int(config['DEFAULT']['long_break_timing'])

        time_spent, time_left = 0, None
        kwargs['timespec'] = str(WORK_TIME)
        for i in range(1, SESSIONS + 1):
            time_left = curses.wrapper(countdown, **kwargs)
            time_spent += int(kwargs['timespec'])
            if time_left != None:
                time_spent -= time_left
                break

            breakkwargs = copy.deepcopy(kwargs)
            if i % LONG_BREAK_TIMING == 0 and i >= LONG_BREAK_TIMING:
                breakkwargs['timespec'] = str(LONG_BREAK)
            else:
                breakkwargs['timespec'] = str(BREAK)
            if i != SESSIONS - 1:
                time_left = curses.wrapper(countdown, **breakkwargs)
            if time_left != None:
                time_spent -= time_left
                break
            time_spent += int(breakkwargs['timespec'])

        db.addTimeElapsed(seconds=time_spent, start_time=start_time)
